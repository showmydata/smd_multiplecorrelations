    ...
    for(j in jSet)
      for(i in iSet) {
        ...
        if(i == j || (i < j && has.lower) || (i > j && has.upper) ) {
          ...
          else if(i < j) localLowerPanel(as.vector(x[, j]), as.vector(x[, i]), as.vector(jdata[, j]), as.vector(jdata[, i]), sp, nvar, lw, smoothness, digits, perc_rank, ...)
          else localUpperPanel(as.vector(x[, j]), as.vector(x[, i]), as.vector(jdata[, j]), as.vector(jdata[, i]), sp, nvar, lw, smoothness, digits, perc_rank, ...)
          if (any(par("mfg") != mfg))
